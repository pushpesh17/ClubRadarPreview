...
export default function SignUpPage() {
  ...
  // Show message if redirected from login page
  useEffect(() => {
    if (messageFromQuery === "account_not_found" && emailFromQuery) {
      toast("Account not found. Please create an account to continue", {
        icon: "ℹ️",
        duration: 5000,
      });
  ...
  }, [emailFromQuery, messageFromQuery]);
  ...
  const handleError = (error: any) => {
    ...
    if (error?.status === 422) {
      if (error?.errors?.[0]?.message?.includes("already exists") || 
          error?.errors?.[0]?.message?.includes("already registered")) {
        userMessage = "This email is already registered. Please sign in instead.";
        toast.error(userMessage, { duration: 5000 });
        setTimeout(() => {
          window.location.href = `/login?email=${encodeURIComponent(emailFromQuery || "")}`;
        }, 2000);
        return;
      }
      userMessage = "Invalid information. Please check your details and try again.";
    } else if (error?.status === 403) {
      userMessage = "Sign-ups are currently disabled. Please contact support or try signing up with Google.";
